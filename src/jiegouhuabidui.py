...
# 逐行同步处理
df_excel = pd.read_excel(r"C:\Users\User\Desktop\pythondata\预后模型评估\10112\bidui\jiegouh\多模型评测\结构化表格\外部验证集评测\外部验证金标准c.xlsx",sheet_name="Sheet2")#'C:\Users\User\Desktop\pythondata\预后模型评估\10112\bidui\jiegouh\病灶明细表与结构化表格初诊基线MRD金标准_cleaned_fixed copy.xlsx')
with open(r'C:\Users\User\Desktop\pythondata\预后模型评估\10112\bidui\jiegouh\多模型评测\结构化表格\外部验证集评测\loraqwen34bthinkingzenjiashuju结构化表格输出.jsonl', 'r', encoding='utf-8') as f:
    for idx, line in enumerate(f):
            ...
            save_comparisons_to_json(result,r"C:\Users\User\Desktop\pythondata\预后模型评估\10112\bidui\jiegouh\多模型评测\结构化表格\外部验证集评测\loraqwen34bthinkingzenjiashuju结构化表格比对c.jsonl")
